 ...
 public void run() {
  ...
  while (true) {
   try {
    ...
    int start = (tokens[1].equals("COORDINATOR")) ? 0 : id + 1;
    if (tokens[0].equals("*"))
     for (int i = start; i < size; i++)
      PseudoServer.list.get(i).out.println(id + "#" + tokens[1]);
    else PseudoServer.list.get(Integer.parseInt(tokens[0])).out.println(id + "#" + tokens[1]);
   } catch (Exception e) {
    e.printStackTrace();
   }
  }
